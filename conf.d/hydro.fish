...
function _hydro_postexec --on-event fish_postexec
    ...
    for code in $last_status
        if test $code -ne 0
            set --local _hydro_error ["$last_status"]
            set --global _hydro_status "$_hydro_color_error$_hydro_error$_hydro_newline$_hydro_color_prompt$_hydro_color_error$hydro_symbol_prompt"
            break
        end
    end
...
function _hydro_prompt --on-event fish_prompt
    ...
    if ! set --query _hydro_git_root[1] || contains -- "$_hydro_git_root" $hydro_ignored_git_paths 
        set --global --export _hydro_skip_git_prompt
    else
